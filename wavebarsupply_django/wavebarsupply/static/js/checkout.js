...
(function () {
// Checkout overlay: opens from the Finalise Purchase button, enables Proceed only
// when every field is filled, keeps the card/CVV numeric, then finalises the
// order over AJAX and shows the order number.
...
function open_overlay() {
  // sync the amount with the (possibly edited) live cart total
...
function close_overlay() {
  if (purchased) {
    window.location.reload();   // cart is now empty — refresh to show that
  } else {
...
if (form) {
  ...
  // numeric-only card number and CVV
  ...
  // expiration: keep digits and auto-insert the slash -> MM/YY
